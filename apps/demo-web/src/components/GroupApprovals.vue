<template>
  <div class="approval-filters">
    <button :class="{ active: sFilter === 'pending' }" @click="sFilter = 'pending'">
      待处理 <span v-if="pendingCount" class="filter-count">{{ pendingCount }}</span>
    </button>
    <button :class="{ active: sFilter === 'all' }" @click="sFilter = 'all'">全部</button>
    <button v-if="currentTeam" :class="{ active: scopeFilter === 'team' }" @click="scopeFilter = 'team'">当前群聊</button>
  ...
  <div v-if="!filtered.length && !loading" class="empty-approval">
  ...
  <div class="approval-list">
    <div v-for="item in filtered" :key="item.id" class="approval-item"
...
const emit = defineEmits<{ resolve: [item: any, action: string]; refresh: []; 'scope-change': [v: string] }>()

const sFilter = ref('pending')
const focusedId = ref('')

const filtered = computed(() => {
  let list = [...props.approvals]
  if (sFilter.value === 'pending') list = list.filter((a) => a.status === 'pending')
  if (props.scopeFilter === 'team' && props.currentTeam) {
    const agentIds = new Set((props.currentTeam.agents || []).map((a: any) => a.id))
    list = list.filter((a) => agentIds.has(a.agent_id))
  }
